...
if grep -q "YOUR_TUNNEL_UUID" cloudflared/config.yml 2>/dev/null || [ -z "$(ls cloudflared/*.json 2>/dev/null)" ]; then
    ...
    echo "  4. Replace YOUR_TUNNEL_UUID in cloudflared/config.yml with your tunnel UUID (it appears twice: the tunnel line and credentials-file line)"
...
export PUBLIC_BETTER_AUTH_URL="${PUBLIC_BETTER_AUTH_URL:-$(grep -m1 '^PUBLIC_BETTER_AUTH_URL=' website/.env 2>/dev/null | cut -d= -f2-)}"
export PUBLIC_BETTER_AUTH_URL="${PUBLIC_BETTER_AUTH_URL:-https://vellum.chromitedev.xyz}"
export PUBLIC_WEBSOCKET_URL="${PUBLIC_WEBSOCKET_URL:-$(grep -m1 '^PUBLIC_WEBSOCKET_URL=' website/.env 2>/dev/null | cut -d= -f2-)}"
export PUBLIC_WEBSOCKET_URL="${PUBLIC_WEBSOCKET_URL:-wss://vellum.chromitedev.xyz/ws}"
export PUBLIC_GOOGLE_ENABLED="${PUBLIC_GOOGLE_ENABLED:-$(grep -m1 '^PUBLIC_GOOGLE_ENABLED=' website/.env 2>/dev/null | cut -d= -f2-)}"
export PUBLIC_GOOGLE_ENABLED="${PUBLIC_GOOGLE_ENABLED:-false}"
